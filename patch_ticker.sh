cat << 'INNER_EOF' > src/components/Ticker.tsx
...
export function Ticker() {
  ...
  return (
    <section className="py-12 border-b border-black/10 overflow-hidden bg-[#FFFFFF]">
      <div className="relative w-full flex items-center">
        ...
        >
          {items.map((partner, idx) => (
            <span
              key={idx}
              className="text-black font-sans font-bold text-3xl tracking-[0.2em] uppercase"
            >
