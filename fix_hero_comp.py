...
export function Hero({ onContactClick }: HeroProps) {
  return (
    <section className="bg-[#139039] w-full pt-32 pb-20 px-6 flex flex-col items-center text-center relative overflow-hidden">
      <motion.div
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true }}
      ...
      >
        <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.8 } } }} className="bg-white text-[#139039] text-xs font-bold tracking-widest uppercase px-4 py-1.5 rounded-full mb-8 inline-flex items-center gap-2 shadow-sm">
        ...
        <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.8 } } }} className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
          ...
          <button className="bg-transparent border border-white text-white px-8 py-4 rounded-full font-bold hover:bg-white hover:text-[#139039] transition-colors">
